...
# test expected codes
EXPECT_CODE = {IP: 28, TLS: 35, URL: 56, INVALID: 0}

def usage():
    print('Usage:   ./checkRules.py ./list.txt [start line] ...')
    print('Example: ./checkRules.py ./list.txt')
    print('         ./checkRules.py ./list.txt 10')
...
def parse(rule):
    ...
    return (TEST_URL % IPLIST[offset] + rule, URL)

def curl(url):
    return subprocess.call(['/usr/bin/curl', '-4', '-I', '-m', '5', url], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
...
if __name__ == '__main__':
    ...
    for rule in lst:
        ...
        # Start testing the url
        (url, url_type) = parse(rule)
        results = [curl(url) for i in range(TEST_COUNT)]
        if any(res == EXPECT_CODE[url_type] for res in results):
            pass
        elif url_type == IP and any(res == EXPECT_CODE[URL] for res in results):
            print(line, '\t', url, '=>', '\033[31mexpecting %d, got %s\033[0m' % (EXPECT_CODE[url_type], ', '.join(results)))
        else:
            print(line, '\t', url, '=>', '\033[31mexpecting %d, got %s\033[0m' % (EXPECT_CODE[url_type], ', '.join(results)))
            log.write('%d: "%s", expecting %d, got %s' % (line, rule, EXPECT_CODE[url_type], ', '.join(results)) + '\n')
